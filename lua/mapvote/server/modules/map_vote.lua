...
function MapVote.isMapAllowed( m )
    ...
    if conf.MapConfig and conf.MapConfig[m] then
        ...
        if cfg.CooldownMinutes and cfg.CooldownMinutes > 0 and MapVote.minutesSinceLastPlayed( m ) < cfg.CooldownMinutes then return false end
    end
...
function MapVote.Start( length )
    local allow = hook.Run( "MapVote_CanStartVote" )
    ...
    hook.Run( "MapVote_VoteStarted", MapVote.state )
